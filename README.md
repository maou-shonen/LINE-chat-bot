...
```
隨機圖片功能設定連結的數量上限好像有限制?
```
LINE限制單次文字最多2000字? 的樣子  
以imgur的例子來看 網址33字 加上一些命令用的字  
大概最多可以輸入5x個左右 如果你需要超過這個數量 暫時沒辦法  
我的解決方案是用網頁設定  
但是由於愛醬我幾乎都是在上班的空閒時間做的 而且我不擅長前端  
所以大概還要好一陣子才會做好

```
回憶系統可以指定某個人嗎Ex.回憶=小見，回答:小見罵了30次幹
```
之前有想過要有  
但是礙於LINE的暱稱有些人會用很多的特殊符號  
不容易輸入 你去點別人@ 又會多出@跟空白  
然後更大的問題是很多人的LINE是舊版的  
沒辦法讀取暱稱  
也許會新增 但不優先
...
看不太懂?
```
內容分組隨機希望增加複數內容共享同一機率，例如A、B共享30%，C、D、E共享70%
```
設定比重 A%15, B%15, C%23, D%23, E%23 即可  
雖然機率會一點差異 但是比重的設定有問題的機會比較少  
因此暫時我不打算調整這部分
...
* 2017.11.06
  * +新增[暫停聊天功能](#暫停聊天)
  * [一些建議的回覆](#一些建議的回覆) 現在愛醬會回覆了 過陣子刪除這欄
* 2017.11.04
